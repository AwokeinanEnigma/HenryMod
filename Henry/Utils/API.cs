...
public static class API
{
    ...
    {
        if (projectileObject.GetComponent<ProjectileController>())
        {
            ProjectileCatalog.getAdditionalEntries += list => list.Add(projectileObject);
            Log.LogD("Registered projectile " + projectileObject.name + " to the projectile catalog!");
            return true;
        }
        Log.LogF("FATAL ERROR:" + projectileObject.name + " failed to register to the projectile catalog!");
        return false;
    }
    /// <summary>
    ...
    public static bool RegisterNewBody(GameObject bodyObject)
    {
        if (bodyObject)
        {
            ...
            Log.LogD("Registered body " + bodyObject.name + " to the body catalog!");
            return true;
        }
        Log.LogF("FATAL ERROR:" + bodyObject.name + " failed to register to the body catalog!");
        return false;
    }
    /// <summary>
    ...
    public static bool RegisterNewMaster(GameObject master)
    {
        if (master && master.GetComponent<CharacterMaster>())
        {
            ...
            Log.LogD("Registered master " + master.name + " to the master catalog!");
            return true;
        }
        Log.LogF("FATAL ERROR: " + master.name + " failed to register to the master catalog!");
        return false;
    }
    /// <summary>
    ...
    public static void CreateEmptySkills(GameObject survivor)
    {
        ...
        else Log.LogF("Tried to create empty skills on a null GameObject!");
    }
    /// <summary>
